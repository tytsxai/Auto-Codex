...
import { CHANGELOG_SOURCE_MODE_LABELS } from '../../../shared/constants';
...
export function ChangelogFilters({
  ...
  const sourceModeDescriptions: Record<ChangelogSourceMode, string> = {
    'tasks': '根据已完成的规格任务生成',
    'git-history': '根据最近提交或标签范围生成',
    'branch-diff': '根据两个分支之间的提交生成'
  };
    ...
    <div className="w-80 shrink-0 border-r border-border overflow-y-auto">
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-3">
          <Label className="text-sm font-medium">变更日志来源</Label>
          <RadioGroup
            value={sourceMode}
          ...
          >
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {sourceModeDescriptions['git-history']}
                </p>
              </div>
            </label>
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {sourceModeDescriptions['branch-diff']}
                </p>
              </div>
            </label>
          </RadioGroup>
        ...
        {sourceMode === 'git-history' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">Git 历史选项</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* History Type */}
              <div className="space-y-2">
                <Label className="text-xs">历史类型</Label>
                <Select
              ...
              {gitHistoryType === 'recent' && (
                <div className="space-y-2">
                  <Label className="text-xs">提交数量</Label>
                  <Input
                    type="number"
                    min={1}
                    max={500}
              ...
              {gitHistoryType === 'tag-range' && (
                <>
                  <div className="space-y-2">
                    <Label className="text-xs">起始标签</Label>
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs">目标标签（可选）</Label>
                    <Select value={gitHistoryToTag || 'HEAD'} onValueChange={(v) => onGitHistoryToTagChange(v === 'HEAD' ? '' : v)}>
                      <SelectTrigger>
                        <SelectValue placeholder="HEAD（最新）" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="HEAD">HEAD（最新）</SelectItem>
                        {tags.map((tag) => (
                          <SelectItem key={tag.name} value={tag.name}>
                            {tag.name}
                          </SelectItem>
              ...
              {gitHistoryType === 'since-version' && (
                <div className="space-y-2">
                  <Label className="text-xs">上一个版本</Label>
        ...
        {sourceMode === 'branch-diff' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">分支比较</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label className="text-xs">基准分支</Label>
              ...
              <div className="space-y-2">
                <Label className="text-xs">对比分支</Label>
                <Select value={compareBranch} onValueChange={onCompareBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder="选择对比分支..." />
                  </SelectTrigger>
                  <SelectContent>
                    {localBranches.map((branch) => (
                      <SelectItem key={branch.name} value={branch.name}>
