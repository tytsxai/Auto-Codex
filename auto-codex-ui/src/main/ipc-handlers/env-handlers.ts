...
${existingVars['AUTO_BUILD_MODEL'] ? `AUTO_BUILD_MODEL=${existingVars['AUTO_BUILD_MODEL']}` : '# AUTO_BUILD_MODEL=gpt-5.2-codex'}

# Reasoning effort override (OPTIONAL) - low/medium/high/xhigh
${existingVars['AUTO_BUILD_REASONING_EFFORT'] ? `AUTO_BUILD_REASONING_EFFORT=${existingVars['AUTO_BUILD_REASONING_EFFORT']}` : '# AUTO_BUILD_REASONING_EFFORT=medium'}
...
`;
  ...
  ipcMain.handle(
    ...
    async (_, projectId: string): Promise<IPCResult<CodexAuthResult>> => {
      ...
      try {
        // Run codex login (device auth) which will open browser for OAuth
        const result = await new Promise<CodexAuthResult>((resolve) => {
        ...
        return {
          success: false,
          error: error instanceof Error ? error.message : 'Failed to invoke Codex login'
        };
      }
    }
  );
